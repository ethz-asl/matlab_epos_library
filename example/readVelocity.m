...
%     end
    ...
    velocity(1) = myhex2dec([dec2hex(out(10),2) dec2hex(out(9),2) dec2hex(out(8),2) dec2hex(out(7),2)]);
    %check if negative
    if velocity(1) > (2^31-1)
        velocity(1) = velocity(1) - 2^32;
    end
...
opCode = uint16(16);
Len_1 = uint16(1);
%DATA = uint16([myhex2dec('28') myhex2dec('20') myhex2dec('00') myhex2dec('02')]);
DATA = uint16([40,32,0,2]);
%DATAforCRC = uint16([myhex2dec('1001') myhex2dec('2028') myhex2dec('0200') myhex2dec('0000')]);
%numberofWordsCRC = 4;
%crc_hex = dec2hex(CRCcalc(DATAforCRC, numberofWordsCRC),4);
%CRC = uint16([myhex2dec(crc_hex(3:4)) myhex2dec(crc_hex(1:2))]);
CRC = uint16([158,206]);
...
%     end
    ...
    velocity(2) = myhex2dec([dec2hex(out(10),2) dec2hex(out(9),2) dec2hex(out(8),2) dec2hex(out(7),2)]);
    %check if negative
    if velocity(2) > (2^31-1)
        velocity(2) = velocity(2) - 2^32;
    end
